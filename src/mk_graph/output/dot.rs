...
use crate::mk_graph::util::{block_name, name_lines, short_name, GraphLabelString};
            ...
            for item in self.items {
                match item.mono_item_kind {
                    MonoItemKind::MonoItemFn { name, body, id: _ } => {
                        ...
                        for (index, decl) in body.as_ref().unwrap().local_decls() {
                        ...
                        let process_blocks =
                            |cluster: &mut Scope<'_, '_>, offset: usize, blocks: &[BasicBlock]| {
                                for (i, b) in blocks.iter().enumerate() {
                                    process_block(cluster, offset + i, b);
                                }
                            };
